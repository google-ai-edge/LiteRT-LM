...
Canonical LiteRT-LM prompt templates and metadata configuration for the Qwen 3.5
model family.
...
To convert and package a Qwen 3.5 Hugging Face model into `.litertlm` format using
`litert-torch`:

```bash
python -m litert_torch.generative.export_hf \
  --model=/path/to/qwen3_5_checkpoint \
  --litert_lm_llm_metadata_override=models/qwen3_5/LlmMetadataProto.pbtext \
