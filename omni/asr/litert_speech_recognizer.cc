...
LiteRtSpeechRecognizer::Create(
  ...
  LITERT_ASSIGN_OR_RETURN(
      auto inputs, compiled_model->CreateInputBuffers(kEncodeSignatureName));
  LITERT_RETURN_IF_ERROR(!inputs.empty());
  LITERT_ASSIGN_OR_RETURN(
      auto outputs, compiled_model->CreateOutputBuffers(kEncodeSignatureName));
  LITERT_RETURN_IF_ERROR(!outputs.empty());
  return std::unique_ptr<LiteRtSpeechRecognizer>(new LiteRtSpeechRecognizer(
      compiled_model, audio_preprocessor, std::move(decoder), std::move(inputs),
      std::move(outputs)));
}
