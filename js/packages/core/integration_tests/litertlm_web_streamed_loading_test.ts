...
describe('LiteRtLm Streamed Loading Tests', () => {
  ...
  describe('Gemma 4 E2B on GPU', () => {
    ...
    it('runs a multi-turn conversation with Gemma 4 E2B on GPU (testing MTP drafter speculative decoding)', async () => {
      if (!initialized) {
        pending('Gemma 4 E2B model not available or GPU not supported');
        return;
      }
      ...
      const response2 = await conversation.sendMessage({
        role: 'user',
        content: 'And what is that number plus 10?',
      });
      ...
      let text = '';
      if (typeof response2.content === 'string') {
        text = response2.content;
      } else if (Array.isArray(response2.content)) {
        text = response2.content
            .filter(item => item.type === 'text')
            .map(item => item.text)
            .join(' ');
