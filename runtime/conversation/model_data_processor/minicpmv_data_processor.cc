...
//   "vit_positions" [1, L, 2] int32             (bucketed ViT (w,h); -1 pad)
...
absl::StatusOr<InputImage> BuildSliceImage(const MinicpmvSlice& sl) {
  ...
  // Contiguous (w,h) for the baked resampler table gather; pad rows are (0,0).
  // Valid rows use the slice's own tgt grid (row-major: col = i % tgt_w).
  // vit_positions hold bucketed 70x70 coords for the ViT absolute pos embed.
  std::vector<int32_t> positions_xy(static_cast<size_t>(num_patches) * 2);
  std::vector<int32_t> vit_positions(static_cast<size_t>(num_patches));
  const int tgt_w = sl.tgt_w > 0 ? sl.tgt_w : 1;
  for (int i = 0; i < num_patches; ++i) {
    const int64_t id = sl.position_ids[i];
    if (id < 0) {
      positions_xy[static_cast<size_t>(i) * 2 + 0] = 0;
      positions_xy[static_cast<size_t>(i) * 2 + 1] = 0;
      // 4899 is the official PAD position embedding index
      vit_positions[i] = 4899;
    } else {
      positions_xy[static_cast<size_t>(i) * 2 + 0] = i % tgt_w;
      positions_xy[static_cast<size_t>(i) * 2 + 1] = i / tgt_w;
      vit_positions[i] = static_cast<int32_t>(id);
    }
  }
  ...
  LITERT_ASSIGN_OR_RETURN(
      ...
      CopyToTensorBuffer<int32_t>(
          ...
          ::litert::Dimensions({1, num_patches})));
...
MinicpmvDataProcessor::ToInputDataVectorImpl(
  ...
  for (const auto& message : messages) {
    ...
    for (const auto& item : message["content"]) {
      ...
      if (item.contains("path")) {
        ...
        if (!absl::Base64Unescape(blob_b64, &image_bytes)) {
          image_bytes = blob_b64;
        }
      }
      break;
    }
  ...
  if (image_bytes.empty()) {
    return absl::InvalidArgumentError(
        "MiniCPM-V: no image bytes in messages.");
  }
  ...
  for (auto& sl : sliced.slices) {
    ...
    const int padded = (sliced.slices.size() == 1) ? np : kMinicpmvMaxPatchLen;
    if (np > padded) {
    ...
    if (padded != np) {
      ...
      std::vector<float> wide(static_cast<size_t>(3) * kPatch * new_w, -1.0f);
  ...
  std::string marker = "<image_soft_token>";
  size_t mpos = rendered.find(marker);
  if (mpos == std::string::npos) {
    marker = "<|image|>";
    mpos = rendered.find(marker);
  }
  if (mpos == std::string::npos) {
    static constexpr std::string_view kUserHeader1 = "<|user|>\n";
    static constexpr std::string_view kUserHeader2 = "<|im_start|>user\n";
    size_t upos = rendered.find(kUserHeader1);
    if (upos != std::string::npos) {
      mpos = upos + kUserHeader1.size();
      marker = "";
    } else {
      upos = rendered.find(kUserHeader2);
      if (upos != std::string::npos) {
        mpos = upos + kUserHeader2.size();
        marker = "";
      } else {
        mpos = 0;
        marker = "";
  ...
  ABSL_ASSIGN_OR_RETURN(auto thumb, BuildSliceImage(sliced.slices[0]));
  ...
  for (int r = 0; r < gy; ++r) {
    for (int col = 0; col < gx; ++col) {
      ...
      push_text("<slice>");
      ABSL_ASSIGN_OR_RETURN(auto sl_img, BuildSliceImage(sliced.slices[sub]));
