...
// reading from "path" or taking inline "bytes".
std::string ExtractFirstImageBytes(const ordered_json& messages) {
  for (const auto& message : messages) {
    ...
    for (const auto& item : message["content"]) {
      ...
      if (item.contains("path")) {
        std::ifstream file(item["path"].get<std::string>(), std::ios::binary);
...
MinicpmvDataProcessor::ToInputDataVectorImpl(
  ...
  const std::string image_bytes = ExtractFirstImageBytes(messages);
