...
class MockSession : public SessionInterface {
 public:
  ...
  MOCK_METHOD(std::optional<SessionDebugInfo>, GetSessionDebugInfo, (),
              (const, override));
};
...
class ConversationTest : public testing::TestWithParam<ConversationTestParams> {
 ...
 protected:
  ...
  std::vector<int> TokenizeSuffix(absl::string_view prefix,
                                  absl::string_view full_text) {
    return ::litert::lm::TokenizeSuffix(*tokenizer_, prefix, full_text);
  }
...
TEST_P(ConversationTest, GetSessionDebugInfo) {
  auto mock_session = CreateMockSession();
  MockSession* mock_session_ptr = mock_session.get();
  EXPECT_CALL(*mock_session_ptr, GetSessionDebugInfo())
      .WillOnce(
          testing::Return(SessionDebugInfo{.capture_dir = "test/capture_dir"}));
  auto mock_engine = CreateMockEngine(std::move(mock_session));
  ASSERT_OK_AND_ASSIGN(
      auto config,
      ConversationConfig::Builder()
          ...
          .Build(*mock_engine));
  ASSERT_OK_AND_ASSIGN(auto conversation,
                       Conversation::Create(*mock_engine, config));

  std::optional<SessionDebugInfo> debug_info =
      conversation->GetSessionDebugInfo();
  ASSERT_TRUE(debug_info.has_value());
  EXPECT_EQ(debug_info->capture_dir, "test/capture_dir");
}
...
TEST_P(ConversationTest, Clone) {
  ...
  ExpectPrefillTokens(*cloned_mock_session_ptr,
                      {TokenizeSuffix("<start_of_turn>user\n"
                                      "Hello<end_of_turn>\n"
                                      "<start_of_turn>assistant\n",
                                      "<start_of_turn>user\n"
                                      "Hello<end_of_turn>\n"
...
TEST(AppendMessageTest,
     CancelProcessDuringSendMessageAsyncWithMergingPendingMessage) {
  // Set up mock Session.
  auto mock_session = std::make_unique<MockSession>();
  MockSession* mock_session_ptr = mock_session.get();
  SessionConfig session_config = SessionConfig::CreateDefault();
  ...
  *session_config.GetMutableLlmModelType().mutable_gemma3() = {};
  ...
  absl::Notification done_1;
  EXPECT_OK(
      conversation->SendMessageAsync(user_message_1,
                                     [&](absl::StatusOr<Message> message) {
                                       EXPECT_OK(message);
                                       done_1.Notify();
                                     },
  ...
  EXPECT_EQ(conversation->GetHistory().size(), 1);
  EXPECT_EQ(conversation->GetHistory().back(), user_message_1);

  // Send second chunk, triggering should_merge == true.
  absl::Notification done_2;
  ...
  EXPECT_OK(
      conversation->SendMessageAsync(user_message_2,
                                     [&](absl::StatusOr<Message> message) {
                                       if (!message.ok()) {
                                         status_2 = message.status();
                                         done_2.Notify();
                                       }
                                     },
                                     {.has_pending_message = true}));

  // Cancel while the second chunk is prefilling.
  EXPECT_CALL(*mock_session_ptr, CancelProcess()).WillOnce([&]() {
    if (stored_callback) {
      stored_callback(Responses(TaskState::kCancelled));
    }
  ...
  EXPECT_THAT(status_2,
              testing::status::StatusIs(absl::StatusCode::kCancelled));
  // History should safely revert back to user_message_1 instead of remaining
  // merged or being deleted.
  EXPECT_EQ(conversation->GetHistory().size(), 1);
  EXPECT_EQ(conversation->GetHistory().back(), user_message_1);
}
...
TEST(AppendMessageTest, Gemma4SyncPrefillPrefaceOnInitAndAlternateRoles) {
  ...
  *session_config.GetMutableLlmModelType().mutable_gemma4() = {};
  ...
  ExpectPrefillTokens(*mock_session_ptr,
                      {Tokenize(*tokenizer, expected_prefill_5)});
  ASSERT_OK_AND_ASSIGN(const Message res5, conversation->SendMessage(
  ...
  ASSERT_OK_AND_ASSIGN(const Message res6, conversation->SendMessage(
                                               nlohmann::json::parse(R"json({
        "role": "tool",
        "content": [
          {
...
TEST_P(ConversationTest,
  ...
  EngineSettings engine_settings = *shared_engine_settings();
