...
absl::Status Conversation::SendMessageAsync(
  ...
  auto run_prefill =
      [this, session_inputs = std::move(session_inputs), internal_callback,
       decode_config = std::move(decode_config),
       optional_args = std::move(optional_args)]() -> absl::Status {
    ABSL_ASSIGN_OR_RETURN(
  ...
  ABSL_RETURN_IF_ERROR(run_prefill());
  std::move(rollback).Cancel();
  return absl::OkStatus();
};
