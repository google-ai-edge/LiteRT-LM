...
nlohmann::ordered_json MergeMessages(const nlohmann::ordered_json& first,
  ...
  if (merged.contains("content") && second.contains("content")) {
    if (merged["content"].is_string() && second["content"].is_string()) {
      merged["content"] = merged["content"].get<std::string>() +
                          second["content"].get<std::string>();
    } else {
      ...
      if (merged["content"].is_array() && second_content.is_array()) {
        for (const auto& item : second_content) {
          if (!merged["content"].empty() &&
              merged["content"].back().is_object() && item.is_object() &&
              merged["content"].back().value("type", "") == "text" &&
              item.value("type", "") == "text" &&
              merged["content"].back().contains("text") &&
              item.contains("text") &&
              merged["content"].back()["text"].is_string() &&
              item["text"].is_string()) {
            merged["content"].back()["text"] =
                merged["content"].back()["text"].get<std::string>() +
                item["text"].get<std::string>();
          } else {
            merged["content"].push_back(item);
          }
        }
      } else {
...
absl::StatusOr<Message> Conversation::SendMessage(const Message& message,
  ...
  auto wait_status = session_->WaitUntilDone();
...
absl::Status Conversation::SendMessageAsync(
...

  {
    ...
    should_merge = was_appending_message && !history_.empty() &&
                   history_.back().is_object() && message.is_object() &&
    ...
    if (!prefill_data.ok()) {
      RevertHistoryLocked(should_merge, previous_last_message,
                          num_messages_added, was_appending_message);
  ...
  absl::Cleanup rollback = [&]() {
    absl::MutexLock lock(history_mutex_);
    RevertHistoryLocked(should_merge, previous_last_message, num_messages_added,
                        was_appending_message);
  };

  if (!config_.enable_rewinding()) {
    ABSL_RETURN_IF_ERROR(session_->Reset());
  }

  absl::AnyInvocable<void()> cancel_callback =
      [this, should_merge, previous_last_message, num_messages_added,
       was_appending_message, cancelled = false]() mutable {
        absl::MutexLock lock(history_mutex_);
...
absl::Status Conversation::RunTextScoringAsync(
  ...
  auto wrapped_callback = [shared_cloned_session,
                           callback = std::move(callback)](
                              absl::StatusOr<Responses> responses) mutable {
    callback(std::move(responses));
  };
  ABSL_ASSIGN_OR_RETURN(auto task_controller,
                        shared_cloned_session->RunTextScoringAsync(
                            target_text, std::move(wrapped_callback),
                            /*store_token_lengths=*/true));
  auto cloned_task_controller = std::make_unique<ClonedSessionTaskController>(
...
absl::StatusOr<std::string> Conversation::RenderMessageIntoString(
  ...
  if (message.is_array()) {
    message_vec.reserve(message.size());
...
absl::StatusOr<std::string> Conversation::GetPrefillTextForMessages(
  ...
  if (!absl::StartsWith(new_string, old_string)) {
    return absl::InternalError(
...
Conversation::GetInputDataVectorForMessages(
  ...
  ABSL_ASSIGN_OR_RETURN(std::vector<InputData> session_inputs,
...
void Conversation::RevertHistoryLocked(bool should_merge,
                                       const Message& previous_last_message,
                                       size_t num_messages_added,
                                       bool was_appending_message) {
  if (should_merge) {
    if (!history_.empty()) {
      history_.back() = previous_last_message;
    }
  } else {
