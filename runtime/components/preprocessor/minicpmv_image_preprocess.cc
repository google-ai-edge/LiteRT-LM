...
int EnsureDivide(int length, int patch) {
  int v = static_cast<int>(
              std::lround(static_cast<double>(length) / patch)) *
          patch;
...
std::pair<int, int> GetRefineSize(int w, int h, int gx, int gy, int scale,
  ...
  auto bg =
      FindBestResize(rw / gx, rh / gy, scale, patch, /*allow_upscale=*/true);
...
std::pair<int, int> GetSlicedGrid(int w, int h, int scale, int max_slice) {
  ...
  for (auto& g : grids) {
    double e = std::abs(
        log_ratio - std::log(static_cast<double>(g.first) / g.second));
    if (e < min_err) {
      best = g;
      min_err = e;
    }
...
std::vector<float> NormalizeAndReshapeByPatch(const std::vector<uint8_t>& rgb,
  ...
  std::vector<float> strip(
      static_cast<size_t>(3) * patch * num_patches * patch);
  ...
  // patches in row-major (patch_row * tgt_w + patch_col), and value =
  // normalized pixel at (patch_row*patch+py, patch_col*patch+px).
    ...
    for (int pc = 0; pc < tgt_w; ++pc) {
      ...
      for (int py = 0; py < patch; ++py) {
        for (int px = 0; px < patch; ++px) {
          ...
          const uint8_t* pxl =
              &rgb[(static_cast<size_t>(img_y) * W + img_x) * 3];
...
// position_ids for a tgt_h x tgt_w slice, bucketed to the pps x pps grid.
...
std::vector<int64_t> ComputePositionIds(int tgt_h, int tgt_w, int pps) {
  ...
  for (int i = 1; i < pps; ++i) {
    boundaries.push_back(static_cast<float>(i) / pps);
  }
  ...
  for (size_t i = 0; i < fh.size(); ++i) {
    bh[i] = bucketize_right(fh[i]);
  }
  for (size_t i = 0; i < fw.size(); ++i) {
    bw[i] = bucketize_right(fw[i]);
  }
...
absl::StatusOr<MinicpmvSliced> PreprocessImageSliced(
  ...
  const int scale = config.scale_resolution, patch = config.patch_size;
  auto grid = GetSlicedGrid(W, H, scale, config.max_slice_nums);
  ...
  if (grid.first == 0) {
    ...
    slice_rgb.push_back(
        PilResizeBicubicRgb(src.data(), W, H, bs.first, bs.second));
    ...
    slice_rgb.push_back(
        PilResizeBicubicRgb(src.data(), W, H, br.first, br.second));
          ...
          for (int x = 0; x < gw; ++x) {
            for (int c = 0; c < 3; ++c) {
              crop[(static_cast<size_t>(y) * gw + x) * 3 + c] =
                  refine[((static_cast<size_t>(i + y) * RW + (j + x)) * 3) +
                         c];
            }
          }
        }
  ...
  // per-patch coords (built in the data processor) and ViT bucketed ids.
  const int pps = config.num_patches_per_side;
