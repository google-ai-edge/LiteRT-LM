...
ml_drift::TensorDescriptor GetTensorDescriptor(const TensorBuffer* tensor_buffer) {
  ...
  ml_drift::BHWC shape(
      dims.size() > 3 ? dims[dims.size() - 4] : 1,
      dims.size() > 2 ? dims[dims.size() - 3] : 1,
      dims.size() > 1 ? dims[dims.size() - 2] : 1,
      dims.size() > 0 ? dims[dims.size() - 1] : 1);
  ml_drift::TensorDescriptor td(
      type, ml_drift::TensorStorageType::BUFFER,
      shape.b == 1 ? ml_drift::Layout::HWC : ml_drift::Layout::BHWC);
  td.SetBHWCShape(shape);
  return td;
}

absl::Status BindTensor(
    const TensorBuffer& tensor_buffer, ml_drift::ValueId tensor_id,
    ml_drift::metal::InferenceContext& inference_context,
    std::vector<ml_drift::metal::MetalSpatialTensor>& shared_tensors) {
  LITERT_ASSIGN_OR_RETURN(auto metal_buffer_handle,
                          tensor_buffer.GetMetalBuffer());
...
absl::Status TopKMetalSampler::SampleToIdAndScoreBuffer(const TensorBuffer& logits_tensor,
  ...
  if (input_handling_ != nullptr && run_inference_func_ != nullptr) {
    ...
    if (ret != 0) {
      return absl::Status(static_cast<absl::StatusCode>(ret),
...
absl::Status TopKMetalSampler::SetInputTensorsAndInferenceFunc(
  ...
  if (input_handling_ == nullptr) {
    ...
    ASSIGN_OR_RETURN(
        ...
        CreateInputHandlingModel(
            tokens_ids_->GetDescriptor(), GetTensorDescriptor(ids_tensor),
    ...
    input_handling_create_info
        .external_mutable_tensors[last_output_token_handle.id] =
    ...
    input_handling_create_info
        .external_mutable_tensors[prev_input_position_handle.id] =
        prev_input_position_handle.tensor_desc;
    input_handling_create_info
        .external_mutable_tensors[input_position_handle.id] =
    ...
    if (mask_tensor) {
      ...
      input_handling_create_info.external_mutable_tensors[mask_handle.id] =
          mask_handle.tensor_desc;
    }

    auto input_handling = std::make_unique<ml_drift::metal::InferenceContext>(
        ml_drift::metal::InferenceContext());
    RETURN_IF_ERROR(input_handling->InitFromGpuModel(
        input_handling_create_info, &input_handling_model,
        env_->device()));
    input_handling_ = std::move(input_handling);
    input_handling_ids_ = {last_output_token_handle.id, input_token_handle.id,
                           prev_input_position_handle.id,
                           input_position_handle.id};
    if (mask_tensor) {
  ...
  RETURN_IF_ERROR(
      input_handling_->SetTensor(input_handling_ids_[0], tokens_ids_.get()));
  RETURN_IF_ERROR(BindTensor(*ids_tensor, input_handling_ids_[1],
                             *input_handling_, shared_tensors_));
  RETURN_IF_ERROR(BindTensor(*prev_input_positions_tensor,
                             input_handling_ids_[2], *input_handling_,
                             shared_tensors_));
  RETURN_IF_ERROR(BindTensor(*input_positions_tensor, input_handling_ids_[3],
                             *input_handling_, shared_tensors_));
  if (mask_tensor != nullptr && input_handling_ids_.size() > 4) {
    RETURN_IF_ERROR(BindTensor(*prev_mask_tensor, input_handling_ids_[4],
                               *input_handling_, shared_tensors_));
    RETURN_IF_ERROR(BindTensor(*mask_tensor, input_handling_ids_[5],
                               *input_handling_, shared_tensors_));
  }
...
int LiteRtTopKMetalSampler_Create(
    ...
    const LiteRtTopKMetalSampler_ActivationDataType* absl_nullable activation_data_type,
    const LiteRtTopKMetalSampler_SamplerParameters* absl_nullable sampler_params,
    LiteRtTopKMetalSampler_Sampler** sampler_out, char** absl_nullable error_msg) {
  if (!env) {
...
int LiteRtTopKMetalSampler_SampleToIdAndScoreBuffer(
    ...
    LiteRtTensorBuffer ids_tensor, const LiteRtTensorBuffer* absl_nullable scores_tensor,
...
int LiteRtTopKMetalSampler_UpdateConfig(
  ...
  SAMPLER_RETURN_AND_SET_ERROR_IF_NOT_OK(
      cpp_sampler->UpdateConfig(cpp_sampler_params, batch_size, cpp_rand_gen),
      error_msg);
  return 0;
}

int LiteRtTopKMetalSampler_CanHandleInput(
    LiteRtTopKMetalSampler_Sampler* sampler) {
  if (!sampler) {
    return false;
  }
...
int LiteRtTopKMetalSampler_HandlesInput(
    LiteRtTopKMetalSampler_Sampler* sampler) {
  if (!sampler) {
    return false;
  }
...
int LiteRtTopKMetalSampler_SetInputTensorsAndInferenceFunc(
  ...
  if (!sampler) {
    SAMPLER_RETURN_AND_SET_ERROR_IF_NOT_OK(
        absl::InvalidArgumentError("Sampler not provided."), error_msg);
  }
  auto cpp_sampler = reinterpret_cast<litert::lm::TopKMetalSampler*>(sampler);
  litert::TensorBuffer cpp_ids_tensor;
  if (ids_tensor) {
    cpp_ids_tensor =
        litert::TensorBuffer::WrapCObject(ids_tensor, litert::OwnHandle::kNo);
  }
  litert::TensorBuffer cpp_prev_input_positions_tensor;
  if (prev_input_positions_tensor) {
    cpp_prev_input_positions_tensor = litert::TensorBuffer::WrapCObject(
        prev_input_positions_tensor, litert::OwnHandle::kNo);
  }
  litert::TensorBuffer cpp_input_positions_tensor;
  if (input_positions_tensor) {
    cpp_input_positions_tensor = litert::TensorBuffer::WrapCObject(
        input_positions_tensor, litert::OwnHandle::kNo);
  }
  litert::TensorBuffer cpp_prev_mask_tensor;
  if (prev_mask_tensor) {
    cpp_prev_mask_tensor = litert::TensorBuffer::WrapCObject(
        prev_mask_tensor, litert::OwnHandle::kNo);
  }
  litert::TensorBuffer cpp_mask_tensor;
  if (mask_tensor) {
    cpp_mask_tensor =
        litert::TensorBuffer::WrapCObject(mask_tensor, litert::OwnHandle::kNo);
  }
  SAMPLER_RETURN_AND_SET_ERROR_IF_NOT_OK(
      cpp_sampler->SetInputTensorsAndInferenceFunc(
          ids_tensor ? &cpp_ids_tensor : nullptr,
          prev_input_positions_tensor ? &cpp_prev_input_positions_tensor
                                      : nullptr,
...
extern "C" int (*LiteRtTopKMetalSampler_UpdateConfig_Static)(
    LiteRtTopKMetalSampler_Sampler* sampler,
    const LiteRtTopKMetalSampler_SamplerParameters* sampler_params,
    int batch_size, void* rand_gen_shared_ptr, char** error_msg);
