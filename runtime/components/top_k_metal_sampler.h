...
SAMPLER_EXPORT int LiteRtTopKMetalSampler_Create(
    ...
    const LiteRtTopKMetalSampler_ActivationDataType* absl_nullable
        activation_data_type,
    const LiteRtTopKMetalSampler_SamplerParameters* absl_nullable
        sampler_params,
 ...
 private:
  ...
  TopKMetalSampler(std::unique_ptr<ml_drift::metal::Environment> env,
      ...
      : TopKGpuSampler(std::move(gpu_info), std::move(create_info),
                       sampler_params, config.batch_size, config.sequence_size,
                       config.vocab_size, config.max_top_k),
        env_(std::move(env)),
