...
class LiteRtLogitMaskRunner {
 ...
 private:
  ...
  explicit LiteRtLogitMaskRunner(Environment* env, HwAccelerators accelerator,
  ...
  // which case it writes to its own output buffer and `copy_runner_` copies
  // that back into `logits`.
  ...
  std::unique_ptr<MaskRunner> copy_runner_;
  ...
  // output and `copy_runner_` copies it back in a second dispatch.
