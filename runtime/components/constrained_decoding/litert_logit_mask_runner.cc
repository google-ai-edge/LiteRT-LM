...
// The graphs are held here, rather than as members of LiteRtLogitMaskRunner,
// so that the class's header does not have to name LambdaModelRunner. That
// type's destructor instantiates a CompiledModelRunner along with the hash
// maps it holds, which every translation unit that destroys a runner (or a
// ConstrainedDecoder owning one) would otherwise emit for itself.
struct LiteRtLogitMaskRunner::MaskGraphs {
  using MaskRunner = tensor::LambdaModelRunner<
      std::function<tensor::TensorsMap(const tensor::TensorsMap&)>>;

  // Lazily compiled mask graphs, indexed by GraphKind.
  std::array<std::unique_ptr<MaskRunner>,
             static_cast<size_t>(GraphKind::kNumGraphKinds)>
      runners;

  // Copy-back runner, only created when `needs_copy_back_` is set:
  //   masked_logits = Add(logits, 0)
  std::unique_ptr<MaskRunner> copy_runner;
};

LiteRtLogitMaskRunner::LiteRtLogitMaskRunner(Environment* env,
                                             HwAccelerators accelerator,
                                             bool force_graph_on_host)
    : env_(env),
      config_{.accelerator = accelerator},
      force_graph_on_host_(force_graph_on_host),
      graphs_(std::make_unique<MaskGraphs>()) {}

LiteRtLogitMaskRunner::~LiteRtLogitMaskRunner() = default;
...
absl::Status LiteRtLogitMaskRunner::EnsureRunner(GraphKind kind) {
  auto& slot = graphs_->runners[static_cast<size_t>(kind)];
  if (slot != nullptr) {
    return absl::OkStatus();
  }
  ...
  slot = std::make_unique<MaskGraphs::MaskRunner>(
...
absl::Status LiteRtLogitMaskRunner::Init() {
  ...
  if (needs_copy_back_) {
    RET_CHECK_NE(env_, nullptr) << "Environment must not be null.";
    graphs_->copy_runner = std::make_unique<MaskGraphs::MaskRunner>(
        *env_, options_,
        tensor::TensorsMap{
            {"logits",
...
absl::Status LiteRtLogitMaskRunner::RunMaskGraph(TensorBuffer& logits,
  ...
  MaskGraphs::MaskRunner* runner =
      graphs_->runners[static_cast<size_t>(kind)].get();
  ...
  if (graphs_->copy_runner == nullptr) {
  ...
  ABSL_RETURN_IF_ERROR(graphs_->copy_runner->SetInput("logits", masked_handle));
  ABSL_RETURN_IF_ERROR(
      graphs_->copy_runner->SetOutput("masked_logits", logits_handle));
  return graphs_->copy_runner->Run();
}
...
absl::Status LiteRtLogitMaskRunner::ApplySlices(
  ...
  if (config_.accelerator != HwAccelerators::kNone) {
    ...
    if (!initialized_ || config_.batch_size != actual_batch ||
      ...
      config_.vocab_size = vocab_size;
      config_.element_type = element_type;
      for (auto& runner : graphs_->runners) {
        runner.reset();
      }
...
int LiteRtLogitMaskRunner::NumCompiledGraphsForTesting() const {
  int count = 0;
  for (const auto& runner : graphs_->runners) {
    if (runner != nullptr) {
      ++count;
    }
  }
