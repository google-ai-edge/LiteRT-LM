...
absl::Status SetEmbeddingFlag(EmbeddingLiteRtLmSettings* settings,
  ...
  if (name == "backend") {
    ...
    settings->vision_backend = std::string(value);
  } else if (name == "audio_backend") {
    settings->audio_backend = std::string(value);
...
absl::Status RunEmbedding(const EmbeddingLiteRtLmSettings& run_settings,
  ...
  const int benchmark_prefill_tokens = run_settings.benchmark_prefill_tokens;
  if (is_benchmark) {
    ...
    if (is_benchmark && benchmark_prefill_tokens > 0) {
      Emit(report_out,
           absl::StrCat("Computing embedding for input prompt: \"",
                        display_prompt, "\" (overridden by ",
                        benchmark_prefill_tokens, " synthetic tokens)"));
    } else {
  ...
  if (is_benchmark) {
    ...
    if (run_settings.report_peak_memory_footprint) {
      const float peak_ram_mb = mem_monitor->GetPeakMemUsageInMB();
      if (peak_ram_mb !=
