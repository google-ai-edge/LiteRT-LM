...
TEST(SignatureSelectionTest, ForwardsSettingsWithoutChangingDefault) {
  ...
  settings.model_path = (std::filesystem::path(::testing::SrcDir()) /
                         "litert_lm/runtime/testdata/test_lm.litertlm").string();
  settings.backend = "cpu";
  auto defaults = CreateEngineSettings(settings);
  ASSERT_TRUE(defaults.ok()) << defaults.status();
  EXPECT_TRUE(defaults->GetMainExecutorSettings().GetSelectedSignatures().empty());
...
TEST(SignatureSelectionTest, PrefillRunnersExcludeInactiveSignatures) {
  ...
  auto excluded = GetPrefillRunnerSetFromModel(**model, "prefill", "input_pos",
                                              {"decode"});
