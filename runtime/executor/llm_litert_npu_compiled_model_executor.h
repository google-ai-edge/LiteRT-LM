...
class LlmLiteRtNpuCompiledModelExecutor : public LlmExecutor {
 ...
 private:
  ...
  // - target_context_size: The target sequence/context length used to resize
  //   dynamic input tensors (e.g. for dynamic shape models). If 0, no
  ...
  static absl::StatusOr<int> AllocateTextDecoderBuffers(
      ...
      std::optional<int> target_context_size = std::nullopt);
