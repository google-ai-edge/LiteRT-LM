...
absl::StatusOr<int> NpuDynamismHelper::ResizeDynamicInputs(
  ...
  if (requested_context_size <= 0) {
    return absl::InvalidArgumentError(
        absl::StrCat("requested_context_size must be positive, got: ",
  ...
  for (size_t i = 0; i < input_names.size(); ++i) {
    ...
    if (absl::StartsWith(name, kDynamicKvCacheKRootName) &&
        new_dims.size() >= 4) {
      new_dims[2] = requested_context_size;
      should_resize = true;
    } else if (absl::StartsWith(name, kDynamicKvCacheVRootName) &&
               new_dims.size() >= 4) {
      new_dims[3] = requested_context_size;
      should_resize = true;
    }
    if (should_resize) {
  ...
  int resolved_context_size = requested_context_size;
  for (size_t i = 0; i < input_names.size(); ++i) {
    absl::string_view name = input_names[i];
    if (absl::StartsWith(name, kDynamicKvCacheKRootName) ||
...
absl::Status NpuDynamismHelper::ResizeDynamicMaskInputs(
  ...
  if (resolved_context_size <= 0) {
    return absl::InvalidArgumentError(
        absl::StrCat("resolved_context_size must be positive, got: ",
...
absl::StatusOr<int> NpuDynamismHelper::ResizeDynamicInputs(
    ...
    absl::string_view /*signature_name*/, int requested_context_size) {
