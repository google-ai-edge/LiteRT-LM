...
//    requested_context_size)` on the text decoder CompiledModel to resize the
//    KV cache to a requested context length. On success, this returns the
//    resolved context size, which is >= requested_context_size (due to hardware
//    bucketing), or an error status on failure.
// 3) Call `ResizeDynamicMaskInputs(aux_compiled_model, signature_name,
//    resolved_context_size)` on the auxiliary CompiledModel, passing in the
//    resolved context size returned from step (2) to ensure mask inputs match
...
struct NpuDynamismHelper {
  ...
  // Returns the resolved context dimension (which is >=
  // requested_context_size due to hardware bucketing) on success, or an error.
  ...
  // Note: `resolved_context_size` should be the resolved context size returned
  // by `ResizeDynamicInputs`.
  static absl::Status ResizeDynamicMaskInputs(
      ::litert::CompiledModel& aux_compiled_model,
      absl::string_view signature_name, int resolved_context_size);
};
