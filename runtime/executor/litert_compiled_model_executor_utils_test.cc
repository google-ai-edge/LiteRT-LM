...
TEST(LlmLiteRTCompiledModelExecutorUtilsTest,
  ...
  EXPECT_THAT(
      FillAttentionMask(*mask_buffer, /*start_timestep=*/5, /*steps=*/2,
                        proto::ATTENTION_MASK_POLICY_VISION_BIDIRECTIONAL,
                        /*token_ids=*/std::nullopt),
...
TEST(LlmLiteRTCompiledModelExecutorUtilsTest,
  ...
  EXPECT_EQ(params.global_policy, proto::ATTENTION_MASK_POLICY_CAUSAL);
  EXPECT_EQ(params.local_policy, proto::ATTENTION_MASK_POLICY_CAUSAL);
...
TEST(LlmLiteRTCompiledModelExecutorUtilsTest,
  ...
  mask_settings->set_attention_mask_policy(
      proto::ATTENTION_MASK_POLICY_VISION_BIDIRECTIONAL);
  mask_settings->set_local_attention_mask_policy(
      proto::ATTENTION_MASK_POLICY_BIDIRECTIONAL);
  mask_settings->set_sliding_window_size(512);

  auto params = GetAttentionMaskParams(&metadata);
  EXPECT_EQ(params.global_policy,
            proto::ATTENTION_MASK_POLICY_VISION_BIDIRECTIONAL);
  EXPECT_EQ(params.local_policy, proto::ATTENTION_MASK_POLICY_BIDIRECTIONAL);
...
TEST(LlmLiteRTCompiledModelExecutorUtilsTest,
  ...
  auto params = GetAttentionMaskParams(&metadata);
  EXPECT_EQ(params.global_policy, proto::ATTENTION_MASK_POLICY_BIDIRECTIONAL);
  EXPECT_EQ(params.local_policy, proto::ATTENTION_MASK_POLICY_BIDIRECTIONAL);
  EXPECT_FALSE(params.sliding_window_size.has_value());
}
