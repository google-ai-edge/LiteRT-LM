...
absl::Status AddSharedValueCacheOptions(const Model& drafter_model,
  ...
  for (absl::string_view input_name : signature.InputNames()) {
    ...
    LITERT_ASSIGN_OR_RETURN(auto producer_type, base_model.GetOutputTensorType(
                                                    "decode", input_name));
...
bool MtpVerificationState::CanVerify(int position,
...
absl::StatusOr<int> MtpVerificationState::CountAcceptedDrafts(
    absl::Span<const int> drafted_tokens, absl::Span<const int> verified_tokens,
    std::optional<int> output_budget) const {
...
LlmLiteRtMtpDrafter::Create(
  ...
  for (bool is_local : {false, true}) {
    ...
    LITERT_ASSIGN_OR_RETURN(auto mask_type,
                            verifier_input_buffers.at(*mask_name).TensorType());
...
absl::StatusOr<std::vector<std::vector<int>>> LlmLiteRtMtpDrafter::Draft(
  ...
  ABSL_ASSIGN_OR_RETURN(
      DraftingResult drafting_result,
      RunDraftingLoop(token_id, activations, activation_index.value_or(-1),
                      constraint_, constraint_state_.get()));
  ...
  ABSL_ASSIGN_OR_RETURN(
      ...
      verification_state_.CountAcceptedDrafts(
          drafting_result.drafted_tokens, verifier_id_vector, output_budget));
