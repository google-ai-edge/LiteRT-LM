...
class LlmLiteRtMtpDrafter {
 public:
  ...
  //    existing CPU/GPU behavior is retained and has the same transition caveat.
...
// requirements for a logical [1,H,D,S] tensor. Other backends are left unchanged;
// unsupported NVIDIA strides are rejected before compiling the reader.
