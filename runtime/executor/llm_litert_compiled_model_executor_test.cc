...
class TestableLlmLiteRtCompiledModelExecutorStatic
 ...
 public:
  ...
  static absl::Status PrefillWithCacheParamsForTest(
      LlmLiteRtCompiledModelExecutorStatic* executor, absl::Span<const int> ids,
      TensorBuffer& cache_params) {
    auto* testable =
        static_cast<TestableLlmLiteRtCompiledModelExecutorStatic*>(executor);
...
TEST(LlmLiteRtCompiledModelExecutorStaticTest,
  ...
  const auto model_path =
      std::filesystem::path(::testing::SrcDir()) / kTestStaticModelPath;
  ASSERT_OK_AND_ASSIGN(auto resources, CreateExecutorModelResourcesLitertLm(
                                           model_path.string()));
  ...
  ASSERT_OK_AND_ASSIGN(
      auto executor,
      LlmLiteRtCompiledModelExecutorStatic::Create(settings, env, *resources));
  const std::vector<int32_t> initial_params(7, -1);
  LITERT_ASSERT_OK_AND_ASSIGN(auto params,
                              CopyToTensorBuffer<int32_t>(initial_params, {7}));
  ...
  for (int session = 0; session < 2; ++session) {
    ...
    ASSERT_OK(
        TestableLlmLiteRtCompiledModelExecutorStatic::
            PrefillWithCacheParamsForTest(executor.get(), {0, 1, 2}, params));
