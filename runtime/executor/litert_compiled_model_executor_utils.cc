...
AttentionMaskParams GetAttentionMaskParams(
  ...
  if (executor_metadata != nullptr &&
    ...
    if (mask_settings.has_attention_mask_policy() &&
        mask_settings.attention_mask_policy() !=
            proto::ATTENTION_MASK_POLICY_UNSPECIFIED) {
      params.global_policy = mask_settings.attention_mask_policy();
    }
    if (mask_settings.has_local_attention_mask_policy() &&
        mask_settings.local_attention_mask_policy() !=
            proto::ATTENTION_MASK_POLICY_UNSPECIFIED) {
      params.local_policy = mask_settings.local_attention_mask_policy();
    } else {
      params.local_policy = params.global_policy;
    }
...
//   attention_mask_policy: Attention mask policy (causal, bidirectional, or
//                          vision bidirectional).
//   token_ids: Token IDs of the context (required for vision bidirectional
//              policy).
static void FillRingBufferPrefillAttentionMask(
  ...
  for (int batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
    for (int step_idx = 0; step_idx < steps; ++step_idx) {
      ...
      if (attention_mask_policy ==
              proto::ATTENTION_MASK_POLICY_VISION_BIDIRECTIONAL &&
        ...
        for (int chunk_key_idx = 0; chunk_key_idx < steps; ++chunk_key_idx) {
          ...
          if (attention_mask_policy ==
              proto::ATTENTION_MASK_POLICY_BIDIRECTIONAL) {
            is_causal = true;
          }
...
static void FillRingBufferDecodeAttentionMask(litert::ElementType elem_type,
...
absl::Status FillAttentionMask(litert::TensorBuffer& mask, int start_timestep,
  ...
  if (is_ring_buffer_prefill) {
    FillRingBufferPrefillAttentionMask(
        ...
        sliding_window_size.value_or(ring_buffer_size), attention_mask_policy,
        token_ids);
    return absl::OkStatus();
  }
