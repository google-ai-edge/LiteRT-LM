...
absl::Status ReduceAndCopyBuffer(TensorBuffer& dst, const TensorBuffer& src,
                                 int reduction_axis) {
  LITERT_ASSIGN_OR_RETURN(
  ...
  src_buffer_ptr += reduction_axis * dst_buffer_size;
...
absl::Status LitertKVCache::ReduceAndCopyFrom(KVCacheInterface& other,
                                              int reduction_axis) {
  ...
  RET_CHECK_LT(reduction_axis, other_litert->batch_size_);
  ...
  for (auto& [input_name, key_cache_buffer] : bank_1_key_cache_buffers_) {
    ...
    RETURN_IF_ERROR(ReduceAndCopyBuffer(
  ...
  for (auto& [input_name, value_cache_buffer] : bank_1_value_cache_buffers_) {
    ...
    RETURN_IF_ERROR(ReduceAndCopyBuffer(
        value_cache_buffer,
        other_litert->bank_1_value_cache_buffers_[input_name], reduction_axis));
  }
  return absl::OkStatus();
}
