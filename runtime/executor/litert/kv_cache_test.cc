...
// litert_lm:google3-only-begin(internal)
// Test model with decode batch size greater than 1.
constexpr char kTestDecodeBatchModelPath[] =
    "google3/third_party/odml/"
    "litert_lm/runtime/e2e_tests/data/smartreply_tinygemma_db3.litertlm";
// litert_lm:google3-only-end
...
// litert_lm:google3-only-begin(internal)
TEST_F(LitertKVCacheTest, CanReduceAndCopyFrom) {
  ASSERT_NO_FATAL_FAILURE(
      SetUpKV(kTestDecodeBatchModelPath, /*inplace_update=*/true));
  EXPECT_EQ(kv_cache_->GetBatchSize(), 3);

  // Write 1 , 2, 3 to each batch in the first key buffer
  {
    ASSERT_OK_AND_ASSIGN(auto buffers, kv_cache_->GetKVCacheBuffers());
    EXPECT_GT(buffers.input_buffers.size(), 2);
    auto& key_cache_buffer = buffers.input_buffers["kv_cache_k_0"];
    LITERT_ASSERT_OK_AND_ASSIGN(
        auto key_cache_data, ReferTensorBufferAsSpan<float>(key_cache_buffer));
    // shape: [3, 1, 2, 64]
    const int offset = 1 * 2 * 64;
    ASSERT_EQ(key_cache_data.size(), 3 * offset);
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < offset; ++j) {
        (key_cache_data)[i * offset + j] = i + 1;
      }
    }
  }

  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<LitertKVCache> prefill_kv_cache,
      LitertKVCache::Create(*env_, *litert_model_, kPrefillSignatureRunner,
                            *compiled_model_, /*inplace_update=*/true));
  ASSERT_EQ(prefill_kv_cache->GetBatchSize(), 1);

  for (int i = 0; i < 3; ++i) {
    ASSERT_OK(prefill_kv_cache->ReduceAndCopyFrom(*kv_cache_, i));

    ASSERT_OK_AND_ASSIGN(auto buffers, prefill_kv_cache->GetKVCacheBuffers());
    auto& key_cache_buffer = buffers.input_buffers["kv_cache_k_0"];
    LITERT_ASSERT_OK_AND_ASSIGN(
        auto key_cache_data, ReferTensorBufferAsSpan<float>(key_cache_buffer));
    const int size = 1 * 2 * 64;
    ASSERT_EQ(key_cache_data.size(), size);
    for (int j = 0; j < size; ++j) {
      EXPECT_EQ((key_cache_data)[j], i + 1);
    }
  }
}

TEST_F(LitertKVCacheTest, CanBroadcastAndCopyFrom) {
  ASSERT_NO_FATAL_FAILURE(
      SetUpKV(kTestDecodeBatchModelPath, /*inplace_update=*/true));
  EXPECT_EQ(kv_cache_->GetBatchSize(), 3);
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      LitertKVCache::Create(*env_, *litert_model_, kPrefillSignatureRunner,
                            *compiled_model_, /*inplace_update=*/true));
  ASSERT_EQ(prefill_kv_cache->GetBatchSize(), 1);
  // Write 123 to the first key buffer.
  {
    ASSERT_OK_AND_ASSIGN(auto buffers, prefill_kv_cache->GetKVCacheBuffers());
    auto& key_cache_buffer = buffers.input_buffers["kv_cache_k_0"];
    LITERT_ASSERT_OK_AND_ASSIGN(
        auto key_cache_data, ReferTensorBufferAsSpan<float>(key_cache_buffer));
    for (int i = 0; i < key_cache_data.size(); ++i) {
      key_cache_data[i] = 123;
    }
  }

  ASSERT_OK(kv_cache_->BroadcastAndCopyFrom(*prefill_kv_cache));
  ASSERT_OK_AND_ASSIGN(auto buffers, kv_cache_->GetKVCacheBuffers());
  auto& key_cache_buffer = buffers.input_buffers["kv_cache_k_0"];
  LITERT_ASSERT_OK_AND_ASSIGN(auto key_cache_data,
                              ReferTensorBufferAsSpan<float>(key_cache_buffer));
  for (int i = 0; i < key_cache_data.size(); ++i) {
    EXPECT_EQ(key_cache_data[i], 123);
  }
}
// litert_lm:google3-only-end
