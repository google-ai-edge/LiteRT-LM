...
def describe_model(
  ...
  if model_info.is_llm_model and model_info.llm:
    llm = model_info.llm
    click.echo("[LLM Capabilities]")
    click.echo(
        "  Supports Function Call: "
        f"{'YES' if llm.supports_function_calling() else 'NO'}"
    )
    click.echo(
        "  Supports Thinking:      "
        f"{'YES' if llm.supports_thinking() else 'NO'}"
    )
    click.echo(
        "  Speculative Decoding:   "
        f"{'YES' if llm.has_speculative_decoding_support() else 'NO'}"
    )
    click.echo(
    ...
    sampler_config = llm.default_sampler_params
    ...
    click.echo(
        "  Is Dynamic Context:     "
        f"{'YES' if llm.is_dynamic_context else 'NO'}"
    )
  ...
  if model_info.is_embedding_model and model_info.embedding:
    ...
    click.echo(
        f"  Max Vision Token Budget: {embed.max_vision_token_budget}"
    )
